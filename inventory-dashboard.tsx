...
function FilterBar({ filters, setFilters, rooftopOptions = [], typeOptions = [], csmOptions = [], enterpriseObjects = [] }) {
  ...
  const activeCount = [filters.enterpriseId, filters.rooftopId, filters.rooftopType, filters.csm, filters.status, filters.after24h !== null ? "x" : null, filters.hasPhotos !== null && filters.hasPhotos !== undefined ? "x" : null, filters.hasVin !== null && filters.hasVin !== undefined ? "x" : null, filters.reasonBucket, filters.inventoryScore, filters.publishing].filter(Boolean).length;
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      {activeCount > 0 && (
        <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
          ...
          {filters.publishing && <Badge label={filters.publishing === "on" ? "Publishing On" : "Publishing Off"} color={filters.publishing === "on" ? "green" : "gray"} />}
        </div>
      )}
    </div>
  );
          ...
          <tbody className={loading && data.length > 0 ? "tbody-loading" : ""}>
            ...
            {data.map((d, i) => (
              <tr key={d.vin} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                ...
                <td style={{ padding: "10px 14px", fontFamily: "monospace", fontSize: 12, borderBottom: "1px solid #f3f4f6" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {d.dealerVinId
                      ...
                      : null}
                    {d.vdpUrl
                      ? <a href={d.vdpUrl} target="_blank" rel="noreferrer" title="Open VDP"
                          style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 6, border: "1px solid #e5e7eb", background: "#f9fafb", color: "#6b7280", textDecoration: "none", transition: "all 0.15s" }}
                          onMouseEnter={e => { e.currentTarget.style.borderColor = "#818cf8"; e.currentTarget.style.color = "#4f46e5"; e.currentTarget.style.background = "#eef2ff"; }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = "#e5e7eb"; e.currentTarget.style.color = "#6b7280"; e.currentTarget.style.background = "#f9fafb"; }}>
                ...
                <td style={{ padding: "10px 14px", borderBottom: "1px solid #f3f4f6", textAlign: "center" }}>
                  {d.vinScore != null
                    ? <span style={{ fontWeight: 700, color: d.vinScore >= 8 ? "#166534" : d.vinScore >= 6 ? "#92400e" : "#991b1b" }}>{Number(d.vinScore).toFixed(1)}</span>
                    : <span style={{ color: "#9ca3af" }}>—</span>}
                </td>
              </tr>
            ))}
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const csvRows = data.map(r => {
        ...
        return [r.enterpriseId, r.enterprise, r.rooftopId, r.name, r.type, r.csm, r.imsIntegrationStatus ?? "", r.publishingStatus ?? "", r.total, r.withPhotos ?? 0, r.deliveredWithPhotos ?? 0, r.pendingWithPhotos ?? 0, r.notProcessedAfter24, rate, r.websiteScore !== null && r.websiteScore !== undefined ? Number(r.websiteScore).toFixed(1) : "", r.avgInventoryScore != null ? Number(r.avgInventoryScore).toFixed(1) : "", ...activeBuckets.map(b => r[b.key] ?? 0), ...reportCols];
      });
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            ...
            <tr ref={row1Ref} style={{ background: "#f9fafb" }}>
              ...
              <th rowSpan={2} onClick={() => handleSort("websiteScore")} style={{ padding: "10px 14px", textAlign: "center", fontWeight: 600, color: "#6d28d9", borderBottom: "2px solid #e5e7eb", whiteSpace: "normal", cursor: "pointer", userSelect: "none", background: "#f5f3ff", position: "sticky", top: 0, zIndex: 2 }}>
                Website Score {sortCol === "websiteScore" ? (sortDir === "asc" ? "↑" : "↓") : <span style={{ color: "#d1d5db" }}>↕</span>}
              </th>
              <th rowSpan={2} onClick={() => handleSort("avgInventoryScore")} style={{ padding: "10px 14px", textAlign: "center", fontWeight: 600, color: "#6d28d9", borderBottom: "2px solid #e5e7eb", whiteSpace: "normal", cursor: "pointer", userSelect: "none", background: "#f5f3ff", position: "sticky", top: 0, zIndex: 2 }}>
            ...
            {rows.map((r, i) => {
              ...
              return (
                <tr key={r.rooftopId || r.name} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                  ...
                  <td style={tdStyle}>{r.type ? <Badge label={r.type} color="blue" /> : <span style={{ color: "#9ca3af" }}>—</span>}</td>
...
function EnterpriseTab({ csmOptions = [], typeOptions = [], hasNotIntegrated = false, hasPublishingDisabled = false, bucketFlags = {}, rows, total, page, pageCount, loading, onPageChange, onDrillDown, filters = DEFAULT_ENTERPRISE_FILTERS, setFilters = (_f) => {}, sortCol, sortDir, onSortChange, reportDates = [], publishing = "all" }: any) {
  ...
  const cols = [
    ...
    ...(showPublishingSplit    ? [{ key: "publishingOnRooftops", label: "Pub. On", numeric: true }, { key: "publishingOffRooftops", label: "Pub. Off", numeric: true }] : []),
  ...
  const activeCount = [filters.csm, filters.accountType, filters.websiteScore, filters.inventoryScore, !!(filters.reportStatus || filters.reportReason)].filter(Boolean).length;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const { data } = await res.json();
      const headers = ["Enterprise ID", "Enterprise Name", "Account Type", "CSM", "Rooftops", ...(showPublishingSplit ? ["Publishing On", "Publishing Off"] : []), ...(showNotIntegrated ? ["Not Integrated"] : []), ...(showPublishingDisabled ? ["Publishing Disabled"] : []), "Total Inventory", "With Photos", "Delivered", "Pending", "Pending VINs >6h", "Pending VINs >6h %", "Avg Website Score", "Avg Inventory Score", ...activeBuckets.map(b => b.label), ...reportDates.map((d: string) => fmtReportDayLabel(d))];
  ...
  return (
    <div>
      ...
      {(filters.csm || filters.accountType || filters.websiteScore || filters.inventoryScore) && (
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            ...
            <tr ref={row1Ref} style={{ background: "#f9fafb" }}>
              ...
              <th rowSpan={2} onClick={() => handleSort("avgWebsiteScore")} style={{ padding: "10px 14px", textAlign: "center", fontWeight: 600, color: "#6d28d9", borderBottom: "2px solid #e5e7eb", whiteSpace: "normal", cursor: "pointer", userSelect: "none", background: "#f5f3ff", position: "sticky", top: 0, zIndex: 2 }}>
              ...
              {[
                ...
                ...(showPublishingSplit    ? [{ key: "publishingOnRooftops", label: "Pub. On" }, { key: "publishingOffRooftops", label: "Pub. Off" }] : []),
            ...
            {rows.map((r, i) => {
              ...
              return (
                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                  ...
                  {showPublishingSplit && (
                    <td style={{ ...tdStyle, textAlign: "center" }}><ClickableNum value={r.publishingOnRooftops ?? 0} color="#166534" onClick={() => onDrillDown({ enterpriseId: r.id, publishing: "on" })} title="Publishing-on VINs" /></td>
                  )}
                  {showPublishingSplit && (
                    <td style={{ ...tdStyle, textAlign: "center" }}>
                      {(r.publishingOffRooftops ?? 0) > 0
                        ? <ClickableNum value={r.publishingOffRooftops} color="#6b7280" onClick={() => onDrillDown({ enterpriseId: r.id, publishing: "off" })} title="Publishing-off VINs" />
                        : <span style={{ color: "#9ca3af" }}>0</span>}
                    </td>
                  )}
                  {showNotIntegrated && (
...
function SummaryTable({ title, rows, colorHeader, filterKey, onDrillDown, onRooftopDrillDown, loading = false, defaultSortCol = "notProcessedAfter24" }) {
  ...
  const totRow = rows.reduce((t, r) => {
    const obj: any = {
      ...
      publishingOnRooftops: (t.publishingOnRooftops ?? 0) + (r.publishingOnRooftops ?? 0),
      publishingOffRooftops: (t.publishingOffRooftops ?? 0) + (r.publishingOffRooftops ?? 0),
    ...
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <DownloadButton onClick={() => {
          const headers = [nameCol, "Enterprises", "Rooftops", ...(showPublishingSplit ? ["Publishing On", "Publishing Off"] : []), "Inventory", "With Photos", "Delivered", "Pending", "Pending VINs >6h", "Pending VINs >6h %", "Avg Website Score", "Avg Inventory Score", ...activeBuckets.map(b => b.label), ...(showPublishing ? ["Publishing Disabled"] : []), "Missing Website", ...(showIntegrated ? ["Not Integrated"] : [])];
          const csvRows = sorted.map(r => [r.label, r.enterpriseCount ?? 0, r.rooftopCount, ...(showPublishingSplit ? [r.publishingOnRooftops ?? 0, r.publishingOffRooftops ?? 0] : []), r.total, r.withPhotos ?? 0, r.deliveredWithPhotos ?? 0, r.pendingWithPhotos ?? 0, r.notProcessedAfter24, (r.pendingWithPhotos ?? 0) === 0 ? 0 : ((r.notProcessedAfter24 / r.pendingWithPhotos) * 100).toFixed(0), r.avgWebsiteScore !== null && r.avgWebsiteScore !== undefined ? Number(r.avgWebsiteScore).toFixed(1) : "", r.avgInventoryScore != null ? Number(r.avgInventoryScore).toFixed(1) : "", ...activeBuckets.map(b => r[b.key] ?? 0), ...(showPublishing ? [r.publishingCount ?? 0] : []), r.missingWebsiteCount ?? 0, ...(showIntegrated ? [r.integratedCount ?? 0] : [])]);
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              ...
              <tr ref={row1Ref} style={{ background: "#f9fafb" }}>
                ...
                {showPublishingSplit && <th rowSpan={2} style={{ ...thBase, textAlign: "center", cursor: "pointer", userSelect: "none" }} onClick={() => handleSort("publishingOnRooftops")}>Pub. On{si("publishingOnRooftops")}</th>}
                {showPublishingSplit && <th rowSpan={2} style={{ ...thBase, textAlign: "center", cursor: "pointer", userSelect: "none" }} onClick={() => handleSort("publishingOffRooftops")}>Pub. Off{si("publishingOffRooftops")}</th>}
              ...
              {sorted.map((r, i) => {
                ...
                return (
                  <tr key={r.label} style={{ background: rowBg }}>
                    ...
                    {showPublishingSplit && (
                      <td style={{ ...td, textAlign: "center" }}><ClickableNum value={r.publishingOnRooftops ?? 0} color="#166534" onClick={() => onRooftopDrillDown({ [filterKey]: r.label, publishingStatus: "Yes" })} title="View in Rooftop View" /></td>
                    )}
                    {showPublishingSplit && (
                      <td style={{ ...td, textAlign: "center" }}>{(r.publishingOffRooftops ?? 0) > 0
                        ? <ClickableNum value={r.publishingOffRooftops} color="#6b7280" onClick={() => onRooftopDrillDown({ [filterKey]: r.label, publishingStatus: "No" })} title="View in Rooftop View" />
            ...
            <tfoot>
              <tr>
                ...
                {showPublishingSplit && <td style={{ ...totTd, textAlign: "center", color: "#166534" }}>{(totRow.publishingOnRooftops ?? 0).toLocaleString()}</td>}
                {showPublishingSplit && <td style={{ ...totTd, textAlign: "center", color: "#6b7280" }}>{(totRow.publishingOffRooftops ?? 0).toLocaleString()}</td>}
          ...
          {activeBuckets.length > 0 && (
            <div style={{ display: "flex", gap: 5, flexWrap: "nowrap", alignItems: "center", opacity: loading ? 0.45 : 1, transition: "opacity 0.2s" }}>
              ...
              {activeBuckets.map((b: { label: string; count: number }) => (
                <span key={b.label} onClick={loading ? undefined : () => onDrillDown({ status: "Not Delivered", after24h: true, hasPhotos: true, reasonBucket: b.label })}
